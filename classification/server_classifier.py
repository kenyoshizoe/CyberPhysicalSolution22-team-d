...
class Classifier:
    ...
    def __call__( self, img ):
        (code, rect), (roi, roi_rect) = detect_roi( img, size_ratio = self.size_ratio, gap_ratio=self.gap_ratio, th_area=self.th_area )
...
def main( port, network, model_name, imshow, imfile, verbose, vs_str, zmq_mode ):
    ...
    server_start( port, dnn, verbose=verbose, imfile=imfile, imshow=imshow, vs_str=vs_str, zmq_mode= zmq_mode)
